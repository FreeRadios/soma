...
int
soma_new_palinsesto_file (soma_controller * c, char *file)
{
  int ret;
  int fl;
  ...
  if (__soma_send_command (c, data, SOMA_STR_NEW_PL))
    {
      __soma_close (c, data);
  ...
  while ((ret = read (fl, buff, sizeof (buff))) > 0)
    __soma_write (c, data, buff, ret);
...
int
soma_new_palinsesto_buffer (soma_controller * c, char *buff, size_t size)
{
  int ret;
  struct __soma_socket_client *data;
...
char *
soma_get_palinsesto (soma_controller * c)
{
  ...
  switch (__soma_send_command (c, d, SOMA_STR_GET_PL))
    {
    case 0:
      break;
...
char *
soma_get_old_palinsesto (soma_controller * c)
{
  struct __soma_socket_client *d;
  int ret;
  char buff[SIZE_BUFFER];
  ...
  switch (__soma_send_command (c, d, SOMA_STR_GET_OLD_PL))
    {
    case 0:
      break;
...
int
soma_old_palinsesto (soma_controller * c)
{
  return __soma_generic (c, SOMA_STR_OLD_PL);
}

int
soma_set_default_palinsesto (soma_controller * c)
{
  return __soma_generic (c, SOMA_STR_DEFAULT_PL);
}
